...
def main() -> int:
    ...
    parser.add_argument("--data", required=False, help="Path to safety_check_*.jsonl")
    parser.add_argument(
        "--models-dir",
    ...
    if args.data:
        ...
        data_path = _latest_data_file(prefix="safety_check_")
    ...
    clf = NaiveBayesTextClassifier(labels=["dangerous", "safe"])
    clf.fit(X_train, y_train)
    y_pred = clf.predict(X_test)

    acc = _accuracy(y_test, y_pred)
    conf = _confusion(y_test, y_pred, labels=["dangerous", "safe"])

    run_id = f"safety_check_{_utc_compact_ts()}"
    ...
    mock_cloud_watch.put_metric(
        "safety_check.accuracy",
        float(acc),
        dimensions={"run_id": run_id, "task": "safety_check"},
    )
